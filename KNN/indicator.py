...
def generate_indicator(stock_code):
    
    file_name = 'sp500_price/'+ stock_code +'.csv' 
    ...
    def get_vr(data_frame):
        return (data_frame['vr'])   
    ...
    '''
    df = read('sp500_price/'+ stock_code +'.csv')
    ...
    Index(['open', 'high', 'low', 'close', 'volume', 'name', 'macd', 'macds',
        ...
        'close_10_ema', 'close_20_ema', 'close_40_ema','vr']
    type(<class 'stockstats.StockDataFrame'>)   
    '''
    # print(df.columns)
    # print(type(df))
    df.to_csv('sp500_indicator/'+ stock_code +'_indicator.csv')
